...
Citizen.CreateThread(function()
  ...
  while true do
    ...
    if(IsControlJustPressed(1, config.interactiveMouse) and (isAdmin or config.debug)) then
      if(mouse == false) then
...
RegisterNetEvent("logs:updateReports")
AddEventHandler("logs:updateReports", function(newArray)
  local array = json.decode(newArray)
  table.insert(reports, array)
end)

RegisterNetEvent("logs:init")
AddEventHandler("logs:init", function(connectionsArray, killsArray, vehiclesArray, weaponsArray, chatArray, reportsArray)
  ...
  reports     = json.decode(reportsArray)
end)
